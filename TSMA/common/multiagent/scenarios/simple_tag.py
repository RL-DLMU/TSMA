...
class Scenario(BaseScenario):
    def make_world(self, args):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.size = 0.075 if agent.adversary else 0.06# 0.06 # 0.05
            agent.accel = 3.0 if agent.adversary else 3.5 # 3.5 # 4.0
            #agent.accel = 20.0 if agent.adversary else 25.0
            agent.max_speed = 1.0 if agent.adversary else 1.2 # 1.2 # 1.3
            if i < num_adversaries:
                agent.action_callback = None  #受控的智能体不需要在这儿获得策略
            else:
                agent.action_callback = self.random_policy  #特工用随机策略
        world.agents[-1].size = 0.03
        world.agents[-1].accel = 4.5
        world.agents[-1].max_speed = 1.5
        # add landmarks
