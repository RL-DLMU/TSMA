...
class PursuitEvade(AbstractMAEnv, EzPickle):
    ...
    def __init__(self, x_size=20, y_size=20, large=False, **kwargs):
        ...
        if large:
            self.n_evaders = kwargs.pop('n_evaders', 50)
            self.n_pursuers = kwargs.pop('n_pursuers', 30)
        else:
            self.n_evaders = kwargs.pop('n_evaders', 30)
            self.n_pursuers = kwargs.pop('n_pursuers', 8)

        self.obs_range = kwargs.pop('obs_range',7)  # can see7 grids around them by default
        ...
        self.max_opponents = kwargs.pop('max_opponents', 30)
        ...
        self.catchr = kwargs.pop('catchr', 1)
        self.caughtr = kwargs.pop('caughtr', 1)

        self.term_pursuit = kwargs.pop('term_pursuit', 50)
        self.term_evade = kwargs.pop('term_evade', -50)
        self.team_dec = kwargs.pop("team_dec", False)

        self.urgency_reward = kwargs.pop('urgency_reward', 0)#-0.1
    ...
    def step(self, actions):
        ...
        """
        # rewards = self.reward()
        idv_rewards, rewards_prey = self.reward()
        ...
        # team_rewards = np.zeros(self.n_pursuers) + self.term_pursuit * ev_remove
        team_rewards = np.zeros(self.n_pursuers) + self.term_pursuit * pursuers_who_remove
        team_rewards = [np.sum(team_rewards)] * self.n_pursuers
        if np.any(np.array(team_rewards) != 0):
            aaa = 0
        ...
        return obslist, rewards, rewards_prey, team_rewards, [done] * self.n_pursuers, info_n
    ...
    def reward(self):
        ...
        prey = sum(rewards)
        agents = self.evaders
        for i, adv in enumerate(self.pursuers):
            dists = [np.sqrt(np.sum(np.square(adv.current_pos - a.current_pos)))
                     for a in agents]
            rewards[i] -= min(dists)
        return np.array(rewards), prey
