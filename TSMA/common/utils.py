...
def make_env(args):
    ...
    if args.scenario_has_diff_rewards:
        if hasattr(scenario, 'done_callback'):
            ...
            done_callback = None
        env = MultiAgentEnv(world, scenario.reset_world, scenario.observation,
                            reward_callback=scenario.individual_reward, team_reward_callback=scenario.team_reward, done_callback=done_callback)
    else:
