...
class irat_mulAgent_td_widv:
    def __init__(self, agent_id, args):
        ...
        self.ob_length_team = args.obs_shape[agent_id]
        # self.ob_length = 3*7
        # self.ob_length_team = 5*8
        ...
        # self.para_actor_start = 1
        # self.para_actor_end = 0.5
        # self.decay_rate = 0.998
        self.e = 0
        self.alpha = 0.01  # 个体间聚合时，mean+alpha*std
        self.miu = -0.0005

        self.idv_num = 2  # 个体策略的数量
        self.actors = []
        self.actor_optimizers = []
        self.criterion = nn.MSELoss(reduction='mean')
        for i in range(self.idv_num):
            self.actors.append(Actor(self.ob_length, self.action_space).to(device))
            self.actor_optimizers.append(torch.optim.Adam(itertools.chain(self.actors[i].parameters()), lr=self.learning_rate,
                                                     eps=1e-5, weight_decay=0.001))
        self.model = self._build_model()
        self.critic_optimizer = torch.optim.Adam(itertools.chain(self.model.parameters()), lr=self.learning_rate, eps=1e-5)


        self.model_team = self._build_model_team()
        self.target_model_team = self._build_model_team()
        self.actor_team = Actor(self.ob_length, self.action_space).to(device)
        # self.criterion = nn.MSELoss(reduction='mean')
        self.critic_optimizer_team = torch.optim.Adam(itertools.chain(self.model_team.parameters()),
                                                      lr=self.learning_rate, eps=1e-5)
        self.actor_optimizer_team = torch.optim.Adam(itertools.chain(self.actor_team.parameters()),
                                                     lr=self.learning_rate, eps=1e-5, weight_decay=0.01)
        # self.actor_optimizer_team = torch.optim.Adam(itertools.chain(self.actor_team.parameters()),
        #                                              lr=self.learning_rate, eps=1e-5)
        # self.aggregation()

        # 保存初始模型状态用于再生正则化
        # self.initial_actors_state = [copy.deepcopy(actor.state_dict()) for actor in self.actors]
        # self.initial_actor_team_state = copy.deepcopy(self.actor_team.state_dict())
        # self.initial_model_state = copy.deepcopy(self.model.state_dict())
        # self.initial_model_team_state = copy.deepcopy(self.model_team.state_dict())

        self.reg_lambda = 0.001  # 再生正则化系数

    def para_actor_start(self, x):
        return max(round(0.5 + np.tanh(self.miu * (x-3500)) / 2, 2), 0)
    ...
    def _build_model_team(self):
        model = ColightNet_team(self.ob_length_team, self.action_space, self.sub_agents).to(device)
        return model

    def compute_l2_norm_difference(self, model, initial_state):
        """计算模型参数与初始状态的L2距离"""
        l2_diff = 0.0
        num_params = 0
        ...
        for name, param in model.named_parameters():
            if name in initial_state:
                param_diff = param.data - initial_state[name]
    ...
    def compute_l2_norm_difference_lowBound(model, initial_state, b=0.0):
        l2_diff = 0.0
        num_params = 0
        ...
        # 计算 L2^2
        l2_norm_squared = l2_diff / num_params

        # 应用转换: (||theta||_2^2 - b).abs() + b
        result = torch.abs(torch.tensor(l2_norm_squared) - b) + b
        return result.item()
    ...
    def _batchwise(self, samples):
        ...
        act_log_prob_team = torch.empty((0), dtype=torch.float32).to(device)
        for item in samples:
            dp = item
            act_log_prob = torch.cat((act_log_prob, torch.tensor(dp[5]).unsqueeze(0).to(device)), 0)
            act_log_prob_team = torch.cat((act_log_prob_team, torch.tensor(dp[7]).to(device)), 0)
            state = torch.tensor(dp[0], dtype=torch.float32).to(device)  # 获取观测信息
            state_team = torch.tensor(dp[8], dtype=torch.float32).to(device)  # 获取观测信息
            batch_list.append(Data(x=state, y=state_team))

            state_p = torch.tensor(dp[4], dtype=torch.float32).to(device)  # 下一观测信息
            state_p_team = torch.tensor(dp[9], dtype=torch.float32).to(device)  # 下一观测信息
            batch_list_p.append(Data(x=state_p, y=state_p_team))
            rewards.append(dp[3])
            rewards_team.append(dp[6])
            actions.append(dp[2])
        batch_t = Batch.from_data_list(batch_list)
        batch_tp = Batch.from_data_list(batch_list_p)
        # TODO reshape slow warning
        rewards = torch.tensor(np.array(rewards), dtype=torch.float32).to(device)
        rewards_team = torch.tensor(np.array(rewards_team), dtype=torch.float32).to(device)
        ...
        if self.sub_agents > 1:
            ...
            rewards_team = rewards_team.view(rewards_team.shape[0] * rewards_team.shape[1])
            actions = actions.view(actions.shape[0] * actions.shape[1])  # TODO: check all dimensions here
        # rewards = rewards.view(rewards.shape[0] * rewards.shape[1])
        # actions = torch.tensor(np.array(actions), dtype=torch.long)
        # actions = actions.view(actions.shape[0] * actions.shape[1])  # TODO: check all dimensions here

        return batch_t, batch_tp, rewards, actions, act_log_prob.permute(1, 0, 2), rewards_team,act_log_prob_team


    def get_action(self, ob, rnn_state, rnn_state_team, test=False):
        """
        ...
        action_dist_team, rnn_state_team = self.actor_team(observation, rnn_state_team)
        action = action_dist_team.sample()
        action_log_probs_team = action_dist_team.log_prob(action).to(device)

        # actor = self.actors[0]
        action_log_probs_list = []
        rnn_state_next_list = torch.empty((0), dtype=torch.float32).to(device)
        for i in range(self.idv_num):
            action_dist, rnn_state_next = self.actors[i](observation, rnn_state[i])
            rnn_state_next_list = torch.cat((rnn_state_next_list, rnn_state_next.unsqueeze(0)), 0)
            # action = action_dist.sample()
            action_log_probs = action_dist.log_prob(action).view(-1).cpu().clone().detach().numpy()
            action_log_probs_list.append(action_log_probs)
        return action.view(-1).cpu().clone().numpy(), action_log_probs_list, rnn_state_next_list, action_log_probs_team.view(-1).cpu().clone().detach().numpy(), rnn_state_team

    def train(self, transition_buffer, plasticity=False):
        action_space = self.action_space
        max_episode_len = len(transition_buffer)
        b_t, b_tp, rewards, actions, act_log_prob, rewards_team, act_log_prob_team = self._batchwise(transition_buffer)
        obs = b_t.x
        ...
        # td_errors = torch.empty((0), dtype=torch.float32).to(device)
        ...
        td_error = (target - out).view(max_episode_len, self.sub_agents, -1)
        td_error = td_error.permute(1, 0, 2)
        td_error = torch.where(td_error>0, td_error, td_error*self.her)

        out_next_team = self.model_team(x=b_tp.y, edge_index='aaa', train=False)
        target_team = rewards_team.view(-1, 1) + self.gamma * out_next_team.view(-1, 1)
        out_team = self.model_team(x=b_t.y, edge_index='aaa', train=True)
        td_error_team = (target_team - out_team).view(max_episode_len, self.sub_agents, -1)
        td_error_team = td_error_team.permute(1, 0, 2)
        td_error_team = torch.where(td_error_team>0, td_error_team, td_error_team*self.her)
        ...
        adv_team = torch.empty((0), dtype=torch.float32).to(device)
        for item_team in td_error_team:  # 每个交叉口的gae
            advantage_team = compute_advantage(self.gamma, self.lmbda, item_team.cpu()).to(device)  # gae
            adv_team = torch.cat((adv_team, advantage_team), 0)
        adv_team = adv_team.view(self.sub_agents, max_episode_len, 1)
        advantage_team = adv_team.permute(1, 0, 2)
        advantage_team = advantage_team.contiguous().view(-1, 1)
        ...
        epochs = 15
        actor_loss_list = [[] for i in range(epochs)]
        for _ in range(epochs):
            for idx, actor in enumerate(self.actors):
                ...
                for item in obs:
                    inp = item.to(device)
                    new_act, rnn_state = actor(inp, rnn_state)
                ...
                ratio = torch.exp(new_log_probs.view(-1, 1) - act_log_prob[idx].detach().reshape(-1, 1))
                ...
                # 添加再生正则化
                # reg_loss = self.compute_l2_norm_difference(actor, self.initial_actors_state[idx])
                # actor_loss = actor_loss + self.reg_lambda * reg_loss

                self.actor_optimizers[idx].zero_grad()
                actor_loss.backward()
                self.actor_optimizers[idx].step()
                actor_loss_list[_].append(actor_loss)

            new_out = self.model(x=b_t.x, edge_index='aaa', train=True)
            critic_loss = torch.mean(F.mse_loss(new_out, target.detach()))

            # 添加再生正则化
            # reg_loss_critic = self.compute_l2_norm_difference(self.model, self.initial_model_state)
            # critic_loss = critic_loss + self.reg_lambda * reg_loss_critic

            self.critic_optimizer.zero_grad()
            critic_loss.backward()
            self.critic_optimizer.step()

            new_probs_team = torch.empty((0), dtype=torch.float32).to(device)
            rnn_state_team = None
            for item_team in obs:
                inp_team = item_team.to(device)
                new_act_team, rnn_state_team = self.actor_team(inp_team, rnn_state_team)
                new_probs_team = torch.cat((new_probs_team, new_act_team.probs), 0)
            new_probs_team = torch.clamp(new_probs_team, min=1e-10, max=1.0)  ### 不为0！！！！
            new_act_p_team = new_probs_team.view(max_episode_len, self.sub_agents, action_space)
            new_log_probs_team = torch.log(torch.gather(new_act_p_team, dim=2, index=actions)).to(device)

            ratio_team = torch.exp(new_log_probs_team.view(-1, 1) - act_log_prob_team.detach().view(-1, 1))
            surr1_team = ratio_team * advantage_team
            surr2_team = torch.clamp(ratio_team, 1-self.eps, 1+self.eps) * advantage_team
            # 策略熵
            log_new_act_team = torch.log(new_act_p_team)
            policy_entropy_team = - torch.sum(new_act_p_team * log_new_act_team, dim=2)
            actor_loss_team = torch.mean(-torch.min(surr1_team, surr2_team)) - 0.01 * torch.mean(policy_entropy_team)
            new_out_team = self.model_team(x=b_t.y, edge_index='aaa', train=True)
            critic_loss_team = torch.mean(F.mse_loss(new_out_team, target_team.detach()))

            # 添加再生正则化
            # reg_loss_actor_team = self.compute_l2_norm_difference(self.actor_team, self.initial_actor_team_state)
            # actor_loss_team = actor_loss_team + self.reg_lambda * reg_loss_actor_team
            # reg_loss_critic_team = self.compute_l2_norm_difference(self.model_team, self.initial_model_team_state)
            # critic_loss_team = critic_loss_team + self.reg_lambda * reg_loss_critic_team

            self.critic_optimizer_team.zero_grad()
            self.actor_optimizer_team.zero_grad()
            critic_loss_team.backward()
            actor_loss_team.backward()
            self.critic_optimizer_team.step()
            self.actor_optimizer_team.step()

        record, drift_idv, drift_team = self.aggregation(actor_loss_list)
        self.e += 1

        # 计算可塑性指标（模型到初始模型的距离）
        # plasticity_actors = torch.tensor([self.compute_l2_norm_difference(actor, self.initial_actors_state[i])
        #                                   for i, actor in enumerate(self.actors)]).mean().item()
        # plasticity_actor_team = self.compute_l2_norm_difference(self.actor_team, self.initial_actor_team_state)
        # plasticity_critic = self.compute_l2_norm_difference(self.model, self.initial_model_state)
        # plasticity_critic_team = self.compute_l2_norm_difference(self.model_team, self.initial_model_team_state)

        torch.cuda.empty_cache()  # 释放显存
        if plasticity:
            return critic_loss.cpu().clone().detach().numpy(), record.cpu().clone().detach().numpy(), drift_idv.cpu().clone().detach().numpy(), drift_team.cpu().clone().detach().numpy(), (plasticity_actors, plasticity_actor_team, plasticity_critic, plasticity_critic_team)
        else:
            return critic_loss.cpu().clone().detach().numpy(), record.cpu().clone().detach().numpy(), drift_idv.cpu().clone().detach().numpy(), drift_team.cpu().clone().detach().numpy()

    def aggregation(self, actor_loss_list):
        # 获取模型参数
        para_actor_team = self.actor_team.state_dict()
        # 创建一个新的字典来存储平均后的参数
        averaged_para_actor = [{} for _ in range(self.idv_num)]
        averaged_para_actor_team = {}
        record = 0
        w = torch.exp(-torch.tensor(actor_loss_list).permute(1, 0).mean(dim=1))
        # values, indices = torch.topk(w, k=self.idv_num)
        # zero_tensor = torch.zeros_like(w)
        # zero_tensor[indices] = w[indices]
        # w = (zero_tensor/zero_tensor.sum()).to(device)
        w = (w/w.sum()).to(device)

        drift_idv = 0
        drift_team = 0
        # 遍历参数并进行平均
        for key in para_actor_team.keys():
            param_list = torch.empty((0), dtype=torch.float32).to(device)
            for idx in range(self.idv_num):
                param = self.actors[idx].state_dict()[key]
                param_list = torch.cat((param_list, param.unsqueeze(0)),dim=0)
            if len(param_list.shape) == 3:
                idvparam_mean = torch.einsum('i, ijk -> jk', w, param_list)
            else:
                idvparam_mean = torch.einsum('i, ij -> j', w, param_list)
            # idvparam_mean = param_list.mean(dim=0)
            idvparam_std = param_list.var(dim=0)

            param_team = para_actor_team[key]
            param_agg = self.para_actor_start(self.e) * idvparam_mean + (1 - self.para_actor_start(self.e)) * param_team  # 个体群体之间，再通过加权聚合
            # 计算偏移量
            drift_idv += torch.norm(idvparam_mean - param_agg)
            drift_team += torch.norm(param_team - param_agg)

            # 聚合后加噪
            std_devs = torch.sqrt(idvparam_std)
            means = torch.zeros_like(std_devs)
            if self.idv_num == 1:
                std_devs = torch.zeros_like(means)

            record += std_devs.sum()
            # 生成一个与方差向量形状相同的随机采样向量
            for idx in range(self.idv_num):
                noisy = torch.normal(mean=means, std=std_devs)
                param_idv = param_agg + self.alpha * noisy  # 个体聚合，mean+alpha*std
                averaged_para_actor[idx][key] = param_idv.to(device)

            # 将平均后的参数存储在新的字典中
            averaged_para_actor_team[key] = param_agg.to(device)
            # 将平均后的参数赋值给两个模型
        for idx in range(self.idv_num):
            self.actors[idx].load_state_dict(averaged_para_actor[idx])
        self.actor_team.load_state_dict(averaged_para_actor_team)
        return record, drift_idv, drift_team
...
class ColightNet_team(nn.Module):
    def __init__(self, input_dim, output_dim, n_node, ):
        super(ColightNet_team, self).__init__()
        # self.model_dict = kwargs
        self.action_space = 5  # 8
        self.n_node = n_node
        self.features = input_dim  # 12
